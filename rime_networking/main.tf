...
resource "aws_acm_certificate" "rime_domain_cert" {
  ...
  domain_name               = var.subdomain_enabled ? "rime.${var.domain}" : var.domain
...
resource "aws_acm_certificate" "fw_domain_cert" {
  ...
  domain_name               = var.subdomain_enabled ? "firewall.${var.domain}" : var.domain
  subject_alternative_names = var.subject_alternative_names
  validation_method         = "DNS"
...
### Cloudflare WAF ACM Certificate
resource "aws_acm_certificate" "cf_domain_cert" {
  count                     = var.cloudflare_waf_enabled ? 1 : 0
...
resource "aws_route53_record" "cf_domain_cert_validation_records" {
  for_each = var.cloudflare_waf_enabled ? {
    for dvo in aws_acm_certificate.cf_domain_cert[0].domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "cf_domain_cert_validation" {
  count                   = var.cloudflare_waf_enabled ? 1 : 0
  certificate_arn         = aws_acm_certificate.cf_domain_cert[0].arn
  validation_record_fqdns = [for record in aws_route53_record.cf_domain_cert_validation_records : record.fqdn]
  provider                = aws.cf
}
