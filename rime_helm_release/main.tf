locals {
  ...
  nginx_geoip2_service_account_name = "rime-${var.namespace}-nginx-geoip2"
...
resource "onepassword_item" "admin_user_password" {
  ...
  lifecycle {
    ignore_changes = [
      # Ignore changes to admin password since users set new one when accessing UI first time
      password,
    ]
  }
}
