locals {
  ...
  # Service account names are used in both the helm chart to create the service account
  # and azure_blob_storage_identity module to link azure blob storage access via OIDC.
...
module "azure_blob_storage_identity" {
  source = "./azure_blob_storage_identity"
  ...
  azure_storage_account_resource_group = var.azure_storage_account_resource_group
...
resource "local_file" "terraform_provided_values" {
  content = templatefile("${path.module}/values_tmpl.yaml", {
    ...
    azure_blob_reader_client_id             = module.azure_blob_storage_identity.azure_blob_reader_client_id
    azure_blob_reader_tenant_id             = module.azure_blob_storage_identity.azure_blob_reader_tenant_id
  ...
  filename = format("%s/values_agent_%s.yaml", local.output_dir, var.namespace)
}
