...
locals {
  managed_node_worker_group_launch_template_per_subnet = merge({
    ...
    disk_encrypted   = true
    disk_size        = var.server_worker_group_root_volume_size
  ...
  managed_node_worker_group_model_testing_launch_template = {
    rime-worker-group-model-testing = merge({
      ...
      disk_encrypted   = true
      disk_size        = var.model_testing_worker_group_root_volume_size
  ...
  managed_node_worker_group_model_testing_large_launch_template = {
    rime-worker-group-model-testing-large = merge({
      name_prefix = "rime-worker-group-model-testing-large"
      ...
      min_capacity     = var.model_testing_worker_group_large_min_size
      desired_capacity = var.model_testing_worker_group_large_desired_size
      max_capacity     = var.model_testing_worker_group_large_max_size
      disk_encrypted   = true
      disk_size        = var.model_testing_worker_group_large_root_volume_size
  ...
  self_managed_worker_group_launch_template_per_subnet = merge({
    ...
    root_encrypted                           = true
    root_volume_size                         = var.server_worker_group_root_volume_size
  ...
  self_managed_worker_group_model_testing_launch_template = merge({
    ...
    root_encrypted          = true
    root_volume_size        = var.model_testing_worker_group_root_volume_size
  ...
  self_managed_worker_group_model_testing_large_instances = merge({
    ...
    root_encrypted          = true
    root_volume_size        = var.model_testing_worker_group_large_root_volume_size
    # Mixed Instance Policy Configurations. May need to tune. Currently we either use all spot or all on-demand.
    # Mixed Instance Policy docs: https://docs.aws.amazon.com/autoscaling/ec2/APIReference/API_MixedInstancesPolicy.html
    on_demand_base_capacity                  = "0"
    on_demand_percentage_above_base_capacity = var.model_testing_worker_group_use_spot ? "0" : "100"
  ...
  eks_asg_tag_list = flatten([
    for name, info in module.eks.node_groups : [
      [
    ...
    ] if strcontains(name, "model-testing")
  ])
...
resource "aws_iam_policy" "ri_prometheus_read_role_access_policy" {
  name        = "ri_prometheus_read_role_access_policy"
...
module "eks" {
  ...
  node_groups = { for k, v in merge(local.managed_node_worker_group_launch_templates, local.managed_node_worker_group_model_testing_launch_template, local.managed_node_worker_group_model_testing_large_launch_template) : k => v if var.use_managed_node_group }
  ...
  workers_additional_policies = concat(var.eks_cluster_node_iam_policies, [aws_iam_policy.ri_product_usage_data_bucket_access_policy.arn, aws_iam_policy.ri_prometheus_read_role_access_policy.arn])
...
resource "aws_autoscaling_group_tag" "eks" {
  for_each               = var.use_managed_node_group ? { for map in local.eks_asg_tag_list : "${map.group_name}%${map.key}" => map } : {}
  autoscaling_group_name = module.eks.node_groups[split("%", each.key)[0]].resources[0].autoscaling_groups[0].name
