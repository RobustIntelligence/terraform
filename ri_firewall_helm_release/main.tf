...
resource "local_file" "helm_values" {
  content = templatefile("${path.module}/values_tmpl.yaml", {
    ...
    domain                      = var.domain
    file_storage_server_image   = var.file_storage_server_image
    file_storage_server_version = var.file_storage_server_version
    integration_secrets_name    = kubernetes_secret.integration_secrets[0].metadata[0].name
    fs_service_account_name     = var.file_storage_server_service_account_name
...
module "blob_store" {
  ...
  service_account_names = [var.file_storage_server_service_account_name]
