locals {
  ...
  # Secret name for the internal firewall agent temporary API key used for registration.
  internal_firewall_agent_api_key_secret_name = "${var.release_name}-internal-firewall-agent-api-key"
}
...
resource "onepassword_item" "admin_user_password" {
  vault    = "z3tkuywibmycgsbjaj2ulazsnm" # Customer Secrets Vault
  title    = "[${var.customer_name}] Admin User Password"
  category = "login"
  username = var.admin_username
  password = var.admin_password == "" ? random_password.admin_password[0].result : var.admin_password
...
resource "kubernetes_secret" "rime-secrets" {
  ...
  data = {
    ...
    adminPassword       = var.admin_password == "" ? random_password.admin_password[0].result : var.admin_password
    rimeLicense         = var.s3_license_enabled ? "" : var.rime_license
    openaiApiKey        = var.openai_api_key
    supportUserPassword = var.initialize_support_user ? random_password.support_user_password[0].result : ""
  }
  depends_on = [kubernetes_namespace.namespace]
}

# Generate a random password for the support user
resource "random_password" "support_user_password" {
  count  = var.initialize_support_user ? 1 : 0
  length = 16
}

resource "onepassword_item" "support_user_password" {
  vault    = "z3tkuywibmycgsbjaj2ulazsnm" # Customer Secrets Vault
  title    = "[${var.customer_name}] Support User Password"
  category = "login"
  username = "support@robustintelligence.com"
  password = random_password.support_user_password[0].result
  tags     = ["terraform-generated"]
}
...
resource "random_string" "internal_agent_generated_api_key" {
  ...
  // This changes the generated API key each time terraform runs.
  // So that we will have fresh API key without worrying its expiration.
  keepers = {
    ts = timestamp()
  }
}
...
# Generate a random API key for the internal firewall agent.
# The control plane init cluster metadata job will create a temporary record for the
# API key that expires after a few days.
resource "random_string" "internal_firewall_agent_generated_api_key" {
  length = 16
...
resource "kubernetes_secret" "internal_firewall_agent_api_key_secret" {
  count = var.manage_namespace && var.internal_firewall_agent_config.enable ? 1 : 0

  metadata {
    name      = local.internal_firewall_agent_api_key_secret_name
    namespace = var.namespace
  }

  data = {
    api-key = random_string.internal_firewall_agent_generated_api_key.result
  }
  depends_on = [kubernetes_namespace.namespace]
}
...
resource "local_file" "helm_values" {
  content = templatefile("${path.module}/values_tmpl.yaml", {
    ...
    internal_firewall_agent_config = {
      enable              = var.internal_firewall_agent_config.enable
      agent_id            = var.internal_firewall_agent_config.agent_id
      api_key_secret_name = local.internal_firewall_agent_api_key_secret_name
      secret_key_path     = "api-key"
    }
