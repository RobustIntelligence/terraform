...
| <a name="input_firewall_system_config"></a> [firewall\_system\_config](#input\_firewall\_system\_config) | Initial firewall system configuration.<br>  This includes information about which model provider (OpenAI or Azure OpenAI)<br>  we will use for evaluation.<br>  It has no customer configuration; the customer can configure certain fields<br>  later with an API call. | `any` | n/a | yes |
