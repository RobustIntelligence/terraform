...
resource "aws_iam_policy" "security_db_gateway_access_policy" {
  name = "security_db_gateway_access_policy_${var.resource_name_suffix}_${var.namespace}"
...
module "iam_assumable_role_with_oidc_for_api_gateway_access" {
  ...
  create_role      = true
  role_name        = "security_db_gateway_access_${var.resource_name_suffix}_${var.namespace}"
