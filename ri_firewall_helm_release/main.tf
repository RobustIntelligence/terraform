...
resource "null_resource" "create_firewall_agent" {
  count = var.create_firewall_agent ? 1 : 0
  provisioner "local-exec" {
    command     = "${path.module}/create_firewall_agent.sh ${var.control_plane_cluster_name} ${var.control_plane_namespace} ${var.platform_address} ${var.agent_override_values_file}"
    interpreter = ["/bin/sh", "-c"]
  }

  triggers = {
    platform_address      = var.platform_address,
    create_firewall_agent = var.create_firewall_agent
  }

  depends_on = [var.dependency_link]
}

data "local_file" "agent_override_values" {
  count    = var.create_firewall_agent ? 1 : 0
  filename = var.agent_override_values_file

  depends_on = [null_resource.create_firewall_agent]
}
...
resource "helm_release" "ri_firewall" {
  ...
  values = [
    ...
    var.create_firewall_agent ? data.local_file.agent_override_values[0].content : "",
  ]

  depends_on = [kubernetes_namespace.namespace, null_resource.create_firewall_agent, data.local_file.agent_override_values]
}
