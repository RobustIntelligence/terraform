...
variable "agent_id" {
  description = "The ID of the agent being deployed. Only required if we are registering the agent. This ID is returned by the CreateFirewallAgent endpoint when the agent is created."
  type        = string
  default     = ""
}
...
variable "api_key" {
  description = "The single-use API key to register the agent."
  type        = string
  default     = ""
  sensitive   = true
}
