...
resource "local_file" "rime_extras" {
  content = templatefile("${path.module}/values_tmpl.yaml", {
    ...
    logscale_hostname                      = var.logscale_hostname
    logscale_ingest_token                  = var.logscale_ingest_token
    install_datadog                        = var.install_datadog
    disable_datadog_metrics                = var.disable_datadog_metrics
    install_logscale_fluentbit             = var.install_logscale_fluentbit
    ...
    install_ri_observability_fluent_bit    = var.install_ri_observability_fluent_bit
